...
class MPII(data.Dataset):
    ...
    def __init__(self, opt, split):
        ...
        self.edges = [[0, 1], [1, 2], [2, 6], [7, 12], [12, 11], [11, 10], [5, 4], [4, 3], [3, 6], [7, 13], [13, 14],
                      [14, 15], [6, 7], [7, 8], [8, 9]]
    ...
    def run_eval(self, results, save_dir, hms=None, test=False):
        if test == True:
            ...
            return pseudo_eval(self.gtFramesSingle, self.convert_eval_format(results, hms), self.gtFramesMulti, self.convert_eval_format_pseudo(results, hms))

    def convert_eval_format_pseudo(self, all_bboxes, hms=None):
        # import pdb; pdb.set_trace()
        detections = []
        data = json.load(open(self.annot_path, 'r'))
        imgs = data['images']
        ...
        for i in range(len(data)):
            img_id = data[i]['image_id']
            if img_id not in self.multiInds:
                continue
            if inds[img_id] not in dct_image_id:
                dct_image_id[inds[img_id]] = []
            lst = []
    ...
    def run_eval_pseudo(self, results, save_dir, hms=None):
        return pseudo_eval(self.gtFrames, self.gtFramesSingle, self.convert_eval_format(results, hms),
                           self.gtFramesMulti, self.convert_eval_format_pseudo(results, hms))
